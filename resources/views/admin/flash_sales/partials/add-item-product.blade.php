            ...
            <x-input type="hidden" name="product_id[]" :value="$product->id" />
                ...
                <div class="row">
                    <div class="col-md-9">
                        <label for=""></label>
                        <div class="mb-3">
                            <span class=""><img width="64" height="64" class="img-fluid" src="{{ asset($product->avatar) }}" alt=""> - {{ $product->name }} - {{ format_price($product->selling_price) }} -
                                Số lượng:
                                @if ($product->type == App\Enums\Product\ProductType::Simple)
                                    {{ $product->qty }}
                                @else
                    ...
                    <div class="col-md-3">
                        <div class="mb-3">
                            ...
                            <x-input data-id="{{ $product->id }}" class="text-center" type="number" name="qty[{{ $product->id }}]" :required="true" min="1" onblur="checkQuantity(this)"/>
                        </div>
                    </div>
                </div>
        </div>
