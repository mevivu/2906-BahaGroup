		...
		@if ($item->type == \App\Enums\Product\ProductType::Simple)
			...
			<div class="product-hover text-center">
				<a style="cursor: pointer;" class="add-to-cart">
					<i onclick="addToCart({{ $item->id }})" class="fa fa-shopping-cart w-50" aria-hidden="true"></i><i
						class="fa fa-arrows-alt w-50" onclick="showDetailProductModal(this, {{ $item->id }})"
						aria-hidden="true"></i>
				</a>
			</div>
		@else
