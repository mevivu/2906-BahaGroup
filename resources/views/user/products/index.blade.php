				...
				@foreach ($categories as $category)
					<div class="d-flex align-items-center fs-12 mb-1">
						<input name="category_slugs[]" value="{{ $category->slug }}" type="checkbox"
							id="category{{ $category->slug }}" class="me-2" autocomplete="off" @if (in_array($category->slug, request('category_slugs', []))) checked @endif>
						...
						@if (isset($co->meta_value['color']))
							<div class="col-2 mb-2">
								<label class="size-filter">
									...
									<input id="checkAll" type="checkbox" name="color_slugs[]" value="{{ $co->slug }}"
										autocomplete="off" @if (in_array($co->slug, request('color_slugs', []))) checked @endif>
				...
				<div class="row me-3">
					@foreach ($sizes->variations as $size)
						<input type="checkbox" class="btn-check" id="btn-check-{{ $size->slug }}" name="size_slugs[]"
							value="{{ $size->slug }}" autocomplete="off" @if (in_array($size->slug, request('size_slugs', [])))
							checked @endif>
						<label
					...
					<div class="price-filter">
						...
						<div class="d-flex align-items-center mb-2">
							<input type="range" id="min-price" name="min_product_price" class="form-range me-2"
								...
								value="{{ $minMax['min_product_price'] }}" oninput="updatePrice()" @if (!request('min_product_price')) disabled @endif>
							<input type="range" id="max-price" name="max_product_price" class="form-range"
								min="{{ $minMax['min_product_price'] }}" max="{{ $minMax['max_product_price'] }}"
								value="{{ $minMax['max_product_price'] }}" oninput="updatePrice()" @if (!request('max_product_price')) disabled @endif>
						</div>
