			...
			<div class="col-md-7 mb-5 mt-5">
				...
				@if (isset($product->productVariations[0]))
						...
						<div class="row">
							<div class="col-md-12">
								<span>{{ $item->attribute->name }}: <strong
										id="attribute_variation_name{{ $item->attribute->slug }}">Black</strong></span><br>
								<x-input id="hiddenAttribute" type="hidden"
									name="attribute_variation_slugs[{{ $item->attribute->slug }}]" />
					...
					<div class="row mt-3">
						...
						<div class="col-md-3"><button id="btnBuyNow" disabled class="btn btn-default w-100 mt-2"><strong>Mua
									ngay</strong></button></div>
					</div>
				@else
					<x-input type="hidden" name="hidden_quantity" :value="$product->qty" />
					<span>Trạng thái: <span class="text-green">{{ $product->qty == 0 ? 'Hết' : 'còn ' . $product->qty }}
							Hàng</span></span>
					<div class="row mt-3">
						<div class="col-md-3">
							<div class="input-group mt-2">
